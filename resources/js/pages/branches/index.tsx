...
function RowActions({
    ...
    return (
        <div className="flex items-center justify-end gap-1.5">
            <CheckPermission
                permissions={[
                    UserPermission.EDIT_ANY_BRANCH,
                    UserPermission.EDIT_ASSOCIATED_BRANCH,
                    UserPermission.EDIT_OWN_BRANCH,
                ]}
            >
            ...
            <CheckPermission
                permissions={[
                    UserPermission.DELETE_ANY_BRANCH,
                    UserPermission.DELETE_ASSOCIATED_BRANCH,
                    UserPermission.DELETE_OWN_BRANCH,
                ]}
            >
...
export default function Index({ businesses = [], users = [] }: IndexProps) {
    ...
    const hasViewAnyBranch = userPermissions.includes(
        UserPermission.VIEW_ANY_BRANCH,
    );

    // Determines create forms permissions:
    const hasCreateAnyBranch = userPermissions.includes(
        UserPermission.CREATE_ANY_BRANCH,
    );
    const hasCreateAssociatedBranch = userPermissions.includes(
        UserPermission.CREATE_ASSOCIATED_BRANCH,
    );
    const hasCreateOwnBranch = userPermissions.includes(
        UserPermission.CREATE_OWN_BRANCH,
    );
    ...
    const filteredBusinesses = React.useMemo(() => {
        if (hasCreateAnyBranch) {
            ...
            return businesses.filter(
                (b) => b.user_id === Number(selectedOwnerId),
            );
        }
        return businesses;
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <FormInput
                        ...
                        onChange={(e) =>
                            setData('opening_time', e.target.value)
                        }
            ...
            render: (b) => {
                ...
                return (
                    <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400">
                        ...
                        <span>
                            {b.opening_time
                                ? `${formatTime(b.opening_time)} - ${formatTime(b.end_time)}`
                                : '24 Jam'}
                        </span>
                    </div>
                );
                    ...
                    renderMobileCard={(branch: Branch) => {
                        ...
                        const canEdit =
                            ...
                            (userPermissions.includes(
                                ...
                                businesses.some(
                                    (b) => b.id === branch.business_id,
                                )) ||
                            (userPermissions.includes(
                                UserPermission.EDIT_OWN_BRANCH,
                            ) &&
                                branch.business?.user_id === user?.id);
                        const canDelete =
                            userPermissions.includes(
                                UserPermission.DELETE_ANY_BRANCH,
                            ) ||
                            ...
                            <div className="flex flex-col gap-3.5 rounded-2xl border border-slate-200 bg-white p-4.5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                                ...
                                <div className="space-y-1.5 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span>
                                            {branch.opening_time
                                                ? `${branch.opening_time.substring(0, 5)} - ${(branch.end_time || '').substring(0, 5)}`
                                                : '24 Jam'}
                                        </span>
                                    </div>
                                </div>
