        ...
        <Modal open={open} onClose={onClose} title="Hapus Item Produk">
            <div className="space-y-4">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                    Apakah Anda yakin ingin menghapus item produk{' '}
                    <strong className="text-slate-900 dark:text-white">
                        {productName}
                    </strong>{' '}
                    beserta seluruh satuan kemasannya? Tindakan ini tidak dapat
                    dibatalkan.
                </p>
            </div>
        ...
        >
            <div className="max-h-[70vh] space-y-6 overflow-y-auto pr-2">
                ...
                <div className="flex flex-col gap-3 rounded-2xl border border-slate-200/60 bg-slate-50 p-4.5 dark:border-slate-800/60 dark:bg-slate-900/40">
                    ...
                    <div className="mt-2 grid grid-cols-1 gap-4 border-t border-slate-200/60 pt-3 text-xs sm:grid-cols-2 dark:border-slate-800/40">
                        <div>
                            <span className="text-[10px] font-semibold text-slate-400 uppercase">
                                Produk Induk (Template)
                            </span>
                ...
                <div className="space-y-3">
                    <h4 className="border-b border-slate-100 pb-2 text-xs font-bold tracking-wider text-slate-400 uppercase dark:border-slate-800">
                        Daftar Unit Kemasan & Tiering Harga
                    </h4>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        {(product.items || []).map((item: any, idx: number) => (
                            <div
                                key={idx}
                                className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 transition-all duration-300 hover:shadow-sm dark:border-slate-800 dark:bg-slate-900/20"
                            >
                                <div className="flex items-center justify-between border-b border-slate-100 pb-2 dark:border-slate-800">
                                    <span className="text-xs font-bold text-slate-800 dark:text-slate-200">
                                        {item.measurement_unit?.name || 'Unit'}{' '}
                                        (
                                    ...
                                    <p className="font-medium text-slate-700 dark:text-slate-300">
                                        {item.is_base_unit ? (
                                            ...
                                            <>
                                                ...
                                                {item.target_measurement_unit
                                                    ?.short_name || 'Base Unit'}
                                            </>
                                        )}
                                    </p>
                                </div>
                                ...
                                <div className="mt-2 space-y-2 border-t border-slate-100 pt-2 dark:border-slate-800">
                                    ...
                                    {(item.price_tierings || []).length ===
                                    0 ? (
                                        <p className="text-[11px] text-slate-400 italic">
                                            Tidak ada tiering harga grosir.
                                        </p>
                                    ) : (
                                        <div className="space-y-1.5">
                                            {item.price_tierings.map(
                                                (tier: any, tIdx: number) => (
                                                    <div
                                                        key={tIdx}
                                                        className="flex items-center justify-between rounded-xl border border-slate-200/50 bg-slate-50 px-2.5 py-1.5 text-xs dark:border-slate-800/40 dark:bg-slate-900/40"
                                                    >
        ...
        <div className="flex items-center justify-end gap-1.5">
            <Tooltip content="Lihat Detail">
                <button
                    ...
                    className="inline-flex cursor-pointer rounded-lg p-1.5 text-emerald-600 transition-colors hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-500/10"
                >
            ...
            <CheckPermission
                permissions={[
                    UserPermission.EDIT_ANY_PRODUCT_ITEM,
                    UserPermission.EDIT_ASSOCIATED_PRODUCT_ITEM,
                    UserPermission.EDIT_OWN_PRODUCT_ITEM,
                ]}
            >
                <Tooltip content="Edit Item Produk">
                    <Link
                        href={`/product-items/${product.id}/edit`}
                        className="inline-flex cursor-pointer rounded-lg p-1.5 text-sky-600 transition-colors hover:bg-sky-50 dark:text-sky-400 dark:hover:bg-sky-500/10"
                    >
            ...
            <CheckPermission
                permissions={[
                    UserPermission.DELETE_ANY_PRODUCT_ITEM,
                    UserPermission.DELETE_ASSOCIATED_PRODUCT_ITEM,
                    UserPermission.DELETE_OWN_PRODUCT_ITEM,
                ]}
            >
                    ...
                    actions={
                        <CheckPermission
                            permissions={[
                                ...
                                UserPermission.CREATE_ASSOCIATED_PRODUCT_ITEM,
                                UserPermission.CREATE_OWN_PRODUCT_ITEM,
                            ]}
                        >
                            <Link
                    ...
                    renderMobileCard={(product: ProductItem) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {(canEdit || canDelete) && (
                                    <div className="mt-1.5 flex gap-2 border-t border-slate-100 pt-3 dark:border-slate-800/60">
                                        <button
                                            onClick={() =>
                                                openDetailModal(product)
                                            }
                                            className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white py-2 text-xs font-semibold text-slate-600 transition-colors hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-500/10"
                                        >
                                        ...
                                        {canEdit && (
                                            <Link
                                                href={`/product-items/${product.id}/edit`}
                                                className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-sky-200 bg-white py-2 text-xs font-semibold text-sky-600 transition-colors hover:bg-sky-50 dark:border-slate-800 dark:bg-slate-900 dark:text-sky-400 dark:hover:bg-sky-500/10"
                                            >
                                        ...
                                        {canDelete && (
                                            <button
                                                ...
                                                className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-rose-200 bg-white py-2 text-xs font-semibold text-rose-600 transition-colors hover:bg-rose-50 dark:border-slate-800 dark:bg-slate-900 dark:text-rose-400 dark:hover:bg-rose-500/10"
                                            >
