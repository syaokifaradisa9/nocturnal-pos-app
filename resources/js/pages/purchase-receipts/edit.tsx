...
export default function Edit({ receipt, suppliers = [], branches = [], users = [], productItemMeasurements = [] }: EditProps) {
    ...
    const ownerId = receipt.branch?.business?.user_id ? String(receipt.branch.business.user_id) : '';

    const [fetchedSuppliers, setFetchedSuppliers] = useState<Supplier[]>(suppliers);
    ...
    const initialRejects: ReceiptReject[] = (receipt.rejects || []).map((reject) => ({
        product_item_measurement_id: String(reject.product_item_measurement_id),
            ...
            <div className="mx-auto max-w-5xl w-full px-4 py-6 sm:px-6 lg:px-8">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 overflow-hidden transition-all duration-300">
                    <div className="border-b border-slate-100 bg-slate-50/50 px-6 md:px-8 py-5 dark:border-slate-800 dark:bg-slate-900/50 flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Edit Penerimaan Barang</h2>
                            <p className="text-xs text-slate-500 dark:text-slate-400">Perbarui detail informasi penerimaan barang ini.</p>
                        </div>
                    </div>
