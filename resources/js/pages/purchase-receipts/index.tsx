...
function DeleteConfirmModal({ open, onClose, onConfirm, receiptNumber, isProcessing }: {
    open: boolean;
    onClose: () => void;
    onConfirm: () => void;
        ...
        <Modal open={open} onClose={onClose} title="Hapus Penerimaan Barang">
            <div className="space-y-4">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                    Apakah Anda yakin ingin menghapus penerimaan barang <strong className="text-slate-900 dark:text-white">{receiptNumber}</strong>? Tindakan ini tidak dapat dibatalkan.
                </p>
            </div>
...
function DetailModal({ open, onClose, receipt }: {
    open: boolean;
    onClose: () => void;
    ...
    return (
        <Modal open={open} onClose={onClose} title="Detail Penerimaan Barang" maxWidth="max-w-4xl">
            <div className="space-y-6">
                ...
                <div className="bg-slate-50 dark:bg-slate-900/40 p-4.5 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 flex flex-col gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Informasi Penerimaan</h4>
                                    ...
                                    {receipt.rejects?.map((reject, idx) => {
                                        ...
                                        return (
                                            <tr key={reject.id} className="hover:bg-slate-50/55 dark:hover:bg-slate-800/30">
                                                <td className="px-4 py-3 text-slate-500">{idx + 1}</td>
                                                <td className="px-4 py-3 font-medium text-slate-805 dark:text-slate-200">{prodName} - {unitName}</td>
...
export default function Index() {
    ...
    const hasConfirmPermission = userPermissions.includes('Konfirmasi Data Penerimaan Barang');
    ...
    const [detailTarget, setDetailTarget] = useState<FullPurchaseReceipt | null>(null);
                        ...
                        <div key={row.id} className="rounded-2xl border border-slate-200 bg-white p-4.5 shadow-sm dark:border-slate-800 dark:bg-slate-900 flex flex-col gap-3.5">
                            ...
                            <div className="flex flex-wrap gap-2 mt-1.5 pt-3 border-t border-slate-100 dark:border-slate-800/60">
                                ...
                                {row.status !== 'Confirmed' && hasConfirmPermission && (
                                    <Link
                                        href={`/purchase-receipts/${row.id}/confirm`}
                                        className="flex-1 min-w-[80px] flex items-center justify-center gap-1.5 rounded-xl border border-emerald-200 bg-white py-2 text-xs font-semibold text-emerald-600 hover:bg-emerald-50 dark:border-slate-800 dark:bg-slate-900 dark:text-emerald-400 dark:hover:bg-emerald-500/10 transition-colors"
                                    >
                                ...
                                {row.status === 'Draft' && (
                                    <Link
                                        href={`/purchase-receipts/${row.id}/edit`}
                                        className="flex-1 min-w-[80px] flex items-center justify-center gap-1.5 rounded-xl border border-amber-200 bg-white py-2 text-xs font-semibold text-amber-600 hover:bg-amber-50 dark:border-slate-800 dark:bg-slate-900 dark:text-amber-400 dark:hover:bg-amber-500/10 transition-colors"
                                    >
