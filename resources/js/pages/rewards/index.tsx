...
import { Plus, Edit2, Trash2, FileSpreadsheet, FileText, Gift } from 'lucide-react';
...
function DeleteConfirmModal({ open, onClose, onConfirm, rewardName, isProcessing }: {
    open: boolean;
    onClose: () => void;
    onConfirm: () => void;
    rewardName: string;
        ...
        <Modal open={open} onClose={onClose} title="Hapus Reward">
            <div className="space-y-4">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                    Apakah Anda yakin ingin menghapus reward <strong className="text-slate-900 dark:text-white">{rewardName}</strong>? Tindakan ini tidak dapat dibatalkan.
                </p>
            </div>
...
/* ──────────────────────── Row Action Menu ──────────────────────── */
function RowActions({ reward, onEdit, onDelete }: {
    reward: Reward;
    ...
    return (
        <div className="flex items-center justify-end gap-1.5">
            <CheckPermission permissions={[UserPermission.EDIT_ANY_REWARD, UserPermission.EDIT_ASSOCIATED_REWARD, UserPermission.EDIT_OWN_REWARD]}>
            ...
            <CheckPermission permissions={[UserPermission.DELETE_ANY_REWARD, UserPermission.DELETE_ASSOCIATED_REWARD, UserPermission.DELETE_OWN_REWARD]}>
...
export default function Index({ businesses = [], users = [] }: IndexProps) {
    ...
    const hasCreateAnyReward = userPermissions.includes(UserPermission.CREATE_ANY_REWARD);
    const hasCreateAssociatedReward = userPermissions.includes(UserPermission.CREATE_ASSOCIATED_REWARD);
    const hasCreateOwnReward = userPermissions.includes(UserPermission.CREATE_OWN_REWARD);
    ...
    const openEditModal = async (reward: Reward) => {
        const ownerId = reward.business?.user_id ? String(reward.business.user_id) : '';
        ...
        if (ownerId) {
            ...
            try {
                const response = await fetch(`/rewards/owner-businesses?user_id=${ownerId}`);
                if (response.ok) {
    ...
    const renderFormFields = (
        ...
        const showBusinessSelector = hasCreateAnyReward 
            ? (selectedOwnerId !== '')
            : (hasCreateAssociatedReward ? (businesses.length > 1) : hasCreateOwnReward ? (businesses.length > 1) : false);

        const displayBusinesses = hasCreateAnyReward ? fetchedBusinesses : businesses;
        ...
        return (
            <div className="space-y-4 pt-2">
                ...
                <FormInput
                    ...
                    onChange={(e) => setFormData('minimum_point', e.target.value)}
    ...
    const columns: ColumnDefinition<Reward>[] = [
        ...
        },
        {
            ...
            render: (r) => <RowActions reward={r} onEdit={openEditModal} onDelete={openDeleteModal} />
        }
    ];
    ...
    return (
        <DashboardLayout title="Reward Penukaran">
            ...
            <div className="mx-auto max-w-7xl px-0 pt-2 pb-6 md:py-6 sm:px-6 lg:px-8">
                    ...
                    <div className="hidden md:block">
                        <div className="flex items-center gap-3 mb-1">
                            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-sky-500/10">
                                <Gift className="h-4.5 w-4.5 text-sky-600 dark:text-sky-400" />
                            </div>
                            <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Data Reward</h1>
                        </div>
                        <p className="ml-12 text-sm text-slate-500 dark:text-slate-400">
                            Kelola reward penukaran poin customer loyalitas sesuai hak akses Anda.
                        </p>
                    </div>

                    <div className="hidden md:flex flex-wrap items-center gap-2">
                        <a
                            href="/rewards/print/excel"
                            target="_blank"
                        ...
                        <CheckPermission permissions={[UserPermission.CREATE_ANY_REWARD, UserPermission.CREATE_ASSOCIATED_REWARD, UserPermission.CREATE_OWN_REWARD]}>
                            <button
                ...
                <Datatable
                    ...
                    renderMobileCard={(reward: Reward) => {
                        ...
                        const canEdit = userPermissions.includes(UserPermission.EDIT_ANY_REWARD) || 
                                        (userPermissions.includes(UserPermission.EDIT_ASSOCIATED_REWARD) && businesses.some(b => reward.business_id === b.id)) ||
                                        (userPermissions.includes(UserPermission.EDIT_OWN_REWARD) && reward.business?.user_id === user?.id);
                        const canDelete = userPermissions.includes(UserPermission.DELETE_ANY_REWARD) || 
                                          (userPermissions.includes(UserPermission.DELETE_ASSOCIATED_REWARD) && businesses.some(b => reward.business_id === b.id)) ||
                                          (userPermissions.includes(UserPermission.DELETE_OWN_REWARD) && reward.business?.user_id === user?.id);

                        return (
                            <div key={reward.id} className="rounded-2xl border border-slate-200 bg-white p-4.5 shadow-sm dark:border-slate-800 dark:bg-slate-900 flex flex-col gap-3.5">
                                {/* Top Line */}
                                ...
                                <div className="grid grid-cols-2 gap-y-2 text-xs">
                                    <div className="text-slate-400">Poin Minimum</div>
                                    <div className="text-slate-700 dark:text-slate-300 text-right font-medium truncate">{reward.minimum_point} Poin</div>
                                    ...
                                    <div className="text-slate-700 dark:text-slate-300 text-right font-medium truncate">
                                        {reward.business ? reward.business.name : '—'}
                                    </div>
                                </div>
            ...
            {/* ─── Create Modal ─── */}
            <Modal open={isCreateModalOpen} onClose={() => { setIsCreateModalOpen(false); resetCreate(); setSelectedOwnerId(''); setFetchedBusinesses([]); clearCreateErrors(); }} title="Tambah Reward Baru">
            ...
            <Modal
                ...
                onClose={() => { setIsEditModalOpen(false); resetEdit(); setSelectedReward(null); setSelectedOwnerId(''); setFetchedBusinesses([]); clearEditErrors(); }}
                title="Edit Reward"
            >
                <form onSubmit={handleEditSubmit}>
                    ...
                    <div className="mt-6 flex justify-end gap-3">
                        <button
                            type="button"
                            onClick={() => { setIsEditModalOpen(false); resetEdit(); setSelectedReward(null); setSelectedOwnerId(''); setFetchedBusinesses([]); clearEditErrors(); }}
                            className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-855 transition-colors"
                        >
                            Batal
                        </button>
                        <button
