    ...
    const handleBranchChange = async (branchId: string) => {
        ...
        try {
            const response = await fetch(
                `/stock-adjustments/branch-batches?branch_id=${branchId}`,
            );
            if (response.ok) {
                ...
                const initialItems = (resData || []).map(
            ...
            <div className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <div className="flex items-center gap-3 border-b border-slate-100 bg-slate-50/50 px-6 py-5 dark:border-slate-800 dark:bg-slate-900/50">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                                Buat rekonsiliasi audit stok fisik baru untuk
                                cabang Anda.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6 p-6">
                        ...
                        <div className="space-y-3 pt-2">
                            ...
                            {isFetching ? (
                                <p className="text-xs text-slate-400 italic dark:text-slate-500">
                                    Membaca data batch cabang...
                                </p>
                            ) : selectedItems.length === 0 ? (
                                <p className="text-xs text-slate-400 italic dark:text-slate-500">
                                    {data.branch_id
                                        ? 'Cabang ini tidak memiliki batch stok aktif untuk diaudit.'
                                        : 'Pilih cabang terlebih dahulu untuk menampilkan batch barang.'}
                                </p>
                            ) : (
                                <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800">
                                    <table className="min-w-full divide-y divide-slate-200 text-left text-xs dark:divide-slate-800">
                                        <thead className="text-slate-550 bg-slate-50 font-semibold uppercase dark:bg-slate-900/60">
                                            <tr>
                                                ...
                                                >
                                                    ...
                                                    <td className="px-2 py-2">
                                                        <input
                                                            ...
                                                            className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-right text-xs font-bold text-slate-900 focus:border-primary focus:ring-1 focus:ring-primary dark:border-slate-800 dark:bg-slate-950 dark:text-white"
                                                            required
                                                        />
                                                    </td>
                                                    <td className="px-2 py-2">
                        ...
                        <div className="flex gap-3 rounded-xl border border-amber-200/60 bg-amber-50 p-4 text-xs text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/10 dark:text-amber-300">
                            <AlertTriangle className="h-5 w-5 shrink-0 text-amber-600 dark:text-amber-500" />
                            <div>
                                <span className="font-bold">Info:</span>{' '}
                                Menyimpan sebagai **Draft** memungkinkan Anda
                                mengedit data opname nanti. Memilih **Terapkan
                                Penyesuaian** akan mengunci data dan secara
                                langsung memutakhirkan level stok fisik di
                                database.
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 border-t border-slate-100 pt-6 dark:border-slate-800">
                            ...
                            <button
                                ...
                                className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800"
                            >
                            ...
                            <button
                                ...
                                className="inline-flex items-center gap-1.5 rounded-xl bg-sky-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-sky-500 disabled:opacity-50"
                            >
